...
class Campaign(models.Model):
    ...
    apply_to = models.CharField(max_length=8, choices=(('Product', 'product'),('Catalog', 'catalog')),
                                default='product',
                                null=False)


    target_product = models.ForeignKey(Product, on_delete=models.SET_NULL, null=True, blank=True)
    target_category = models.ForeignKey(Catalog, on_delete=models.SET_NULL, null=True, blank=True)
...
class Coupon(models.Model):
    minimum_cart_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True)
    discount_rate = models.IntegerField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now_add=True)
